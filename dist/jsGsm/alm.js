...
function translate() {
    $("#activeAlarm").text(top.DISPLAY.Alarm.CurrentAlarm);
}
...
function parseAlarm(record) {
    ...
    for (var i = record.multiData.length; i > 0; i--) {
        var state = '<span class="label label-important">告警</span>'
        dataArr.push([record.multiData[i - 1].AlarmCnName, record.multiData[i - 1].AlarmRaiseTime, record.multiData[i - 1].AlarmRaiseCause, state]);
    }
